...
class Pagination
{
	...
	public function showPagination($link = null)
	{
		...
		if (empty($params)) {
			...
			$link = 'index.php?module=backend&controller=' . $params['controller'] . '&action=index&currentStatus=all';
		}
		$paginationHTML = '';
		if ($this->totalPage > 1) {
	...
	public function FrontendPagination($link = null)
	{
		// Pagination
		$params = array_merge($_POST, $_GET);
		$queries = [];
		...
		if (empty($params)) {
			...
			$link = 'book-list-trang';
			if(isset($params['category_id'])) {
				$infoParams['id'] = $params['category_id'];
				
				$categoryModel = new CategoryModel();
				$item = $categoryModel->infoItem($infoParams);
				$categoryName = URL::filterURL($item['name']);
				$cateID = $item['id'];
				// filterURL
				$link = "$categoryName-$cateID-trang";
			}
		}
		$paginationHTML = '';
		if ($this->totalPage > 1) {
			...
			if ($this->currentPage > 1) {
				$start 	= '<li class="page-item"><a class="page-link" href="' . $link . '-1.html"><i class="fa fa-angle-double-left"></i></a></li>';
				$prev 	= '<li class="page-item"><a class="page-link" href="' . $link . '-' . ($this->currentPage - 1) . '.html"><i class="fa fa-angle-left"></i></a></li>';
			}
			...
			if ($this->currentPage < $this->totalPage) {
				$next 	= '<li class="page-item"><a class="page-link" href="' . $link . '-' . ($this->currentPage + 1) . '.html"><i class="fa fa-angle-right"></i></a></li>';
				$end 	= '<li class="page-item"><a class="page-link" href="' . $link . '-' . $this->totalPage . '.html"><i class="fa fa-angle-double-right"></i></a></li>';
			}
			...
			for ($i = $startPage; $i <= $endPage; $i++) {
				if ($i == $this->currentPage) {
					...
					$listPages .= '<li class="page-item"><a class="page-link" href="' . $link . '-' . $i . '.html">' . $i . '</a></li>';
				}
			}
